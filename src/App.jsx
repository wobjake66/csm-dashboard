...
function BobView({filterCoach, bobRaw}) {
  ...
  const liveCsms = (bobRaw && Object.keys(bobRaw.bob||{}).length > 0)
    ? Object.entries(bobRaw.bob).map(([name, d]) => {
        ...
        return {n:name, c:d.coach||fallback.c||"", boq:d.boq, lcm:d.lcm, net:d.net, ret:d.ret,
          mca:fallback.mca, mcc:fallback.mcc, mch:fallback.mch,
          bca:fallback.bca, bcc:fallback.bcc, bch:fallback.bch};
      }).filter(c => c.boq > 0)
    : BOB_CSMS;
  ...
  const coachesVisible = filterCoach ? [filterCoach] : Object.keys(liveCoachTotals);
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    let latestEmail=[], latestCad=[], latestDue=[], latestOntime=[], latestHistory=[], latestSkipped=[], latestBob=[];
    ...
    function loadAll() {
      ...
      ]).then(([revRows, emailRows, cadRows, dueRows, ontimeRows, historyRows, skippedRows, bobRows]) => {
    ...
    <div style={{fontFamily:"Nunito Sans,sans-serif",background:"#F4F6FB",minHeight:"100vh",fontSize:13}}>
      ...
      {hasData&&(
        <div style={{padding:"20px 24px"}}>
          ...
          {tab==="bob"&&<BobView filterCoach={filterCoach} bobRaw={bobRaw}/>}
